...
public class FavoriteServlet extends HttpServlet {
    ...
    private void getFolders(HttpServletRequest req, HttpServletResponse resp, int userId)
        ...
        // 获取搜索参数
        String searchKeyword = Optional.ofNullable(req.getParameter("search"))
                                      .orElse("").trim();
        ...
        try (PrintWriter out = resp.getWriter()) {
            ...
            try {
                ...
                if (!searchKeyword.isEmpty()) {
                    System.out.println("执行模糊搜索，关键词: " + searchKeyword);
                    folders = folderService.searchFolders(userId, searchKeyword);
                } else {
                    System.out.println("获取全部收藏夹");
                    folders = folderService.getUserFolders(userId);
                }

                // 构建JSON响应
                if (folders != null && !folders.isEmpty()) {
                    System.out.println("查询到收藏夹数量: " + folders.size());
                    for (FavoriteFolder folder : folders) {
                        JSONObject json = new JSONObject();
                        json.put("id", folder.getId());
                        json.put("folderName", folder.getFolderName());
                        json.put("description", folder.getDescription() != null ? folder.getDescription() : "");
                        jsonArray.put(json);
                    }
                } else {
                    System.out.println("未找到匹配的收藏夹");
                }
                
                out.print(jsonArray.toString());
                System.out.println("返回数据: " + jsonArray.toString(2)); // 美化输出日志
                ...
                errorJson.put("message", "操作失败: " + e.getMessage());
                out.print(errorJson.toString());
            }
        } catch (IOException e) {
            System.err.println("输出流异常: " + e.getMessage());
            throw new IOException("响应输出失败", e);
        }
    }
    ...
    private void editFolder(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (fid != null && name != null && !fid.trim().isEmpty() && !name.trim().isEmpty()) {
            try {
                ...
                f.setFolderName(name.trim());
                f.setDescription(description); // 添加描述字段
                // 获取原始文件夹信息，确保userId也被正确设置
                FavoriteFolder originalFolder = folderService.getFolderById(Integer.parseInt(fid.trim()));
                if (originalFolder != null) {
                    f.setUserId(originalFolder.getUserId());
                }
...
*/
    ...
    private void removeFromFavorite(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            System.out.println("移除收藏请求参数 - folderId: " + folderIdStr + ", petId: " + petIdStr);
            ...
            System.err.println("移除收藏异常: " + e.getMessage());
            e.printStackTrace();
            result.put("success", false);
